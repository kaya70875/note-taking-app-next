    ...
    return (
        <div className='flex flex-col w-1/2 md:w-3/4 xs:w-[90%] max-w-xl gap-4 p-6 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-lg bg-neutral-50 dark:bg-neutral-700 border'>
            ...
            <div className='buttons w-full flex justify-end gap-4'>
                <button onClick={() => onClose(false)} className='flex items-center justify-center p-3 rounded-lg bg-neutral-200 dark:bg-neutral-600'>
                    <p>Cancel</p>
                </button>

                <button onClick={onDelete} className='flex items-center justify-center p-3 rounded-lg bg-red-500 active:opacity-80 disabled:opacity-50' disabled={loading}>
                    <p className='text-neutral-200'>Delete Note</p>
                </button>
            </div>
        </div>
    )
