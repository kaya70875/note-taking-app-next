...
export default function Sidebar() {
    ...
    const activeItem = sidebarItems.find(item => pathName.startsWith(item.route))?.name || 'All Notes';
    const activeTag = tags?.find(tag => searchParams.get('tag') === tag);
                ...
                <section className="sidebar-top">
                    <ul className='flex flex-col gap-4'>
                        {sidebarItems.map(item => (
                            <Link href={item.name === 'Archived Notes' ? '/archived' : '/notes'} key= {item.name} className={`flex items-center justify-between p-2 cursor-pointer ${!searchParams.has('tag') && activeItem === item.name ? 'activeItem dark:bg-neutral-700' : ''}`}>
